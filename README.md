...
A full-stack productivity tracker — sleep, focus sessions, calendar, checklists — built end-to-end as a personal learning project. Android client, REST API backend, WebAssembly analytics dashboard, fully deployed on AWS.
...
| `backend/` | REST API — Axum, sqlx + Postgres, JWT auth (HS256, `iss`/`aud` pinned, per-user `token_version` revocation), Argon2id passwords, SSE for realtime sync |
...
**Backend** — Axum 0.7, sqlx, PostgreSQL, JWT (jsonwebtoken), Argon2id, tower_governor for per-IP rate limiting, tower-http security headers, broadcast channels for SSE fan-out.

**Web dashboard** — Leptos 0.8 (CSR), Tailwind, charming/leptos-chartistry, gloo-net.
