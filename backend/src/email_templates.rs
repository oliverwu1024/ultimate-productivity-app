...
struct EmailCopy {
    subject: &'static str,
    greeting: &'static str,
    /// The "tap the link to do X, because Y" sentence, ending with a colon.
    /// Says "link" (not "button") so it reads correctly in the plaintext part,
    /// which has no button — only the raw URL.
    body: &'static str,
    button: &'static str,
